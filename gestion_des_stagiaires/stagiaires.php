...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['save'])) {
        ...
        if (isset($_POST['id_stagiaire']) && (int) $_POST['id_stagiaire'] > 0) {
            ...
            // Always regenerate — server is authoritative; field is display-only in the form
            $year = date('Y', strtotime($di));
            $st = $pdo->prepare("SELECT COALESCE(MAX(CAST(SUBSTRING_INDEX(num_inscri, '-', -1) AS UNSIGNED)), 0) FROM stagiaires WHERE num_inscri LIKE ? AND num_inscri REGEXP '^INS-[0-9]{4}-[0-9]{5}$'");
            $st->execute(['INS-' . $year . '-%']);
            $mat = 'INS-' . $year . '-' . str_pad((string)((int)$st->fetchColumn() + 1), 5, '0', STR_PAD_LEFT);
            // Insertion avec protection contre les doublons base de données (filet de sécurité)
            try {
                $pdo->prepare('INSERT INTO stagiaires (num_inscri, cin, nom, prenom, date_naissance, adresse, email, telephone, telephone_parent, nom_tuteur, photo, date_inscription, id_classe, remise_mensuelle, sexe) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)')
                    ->execute([$mat, $cin === '' ? null : $cin, $nom, $prenom, $dn, $adr === '' ? null : $adr, $emNull, $tel === '' ? null : $tel, $telp === '' ? null : $telp, $tuteur === '' ? null : $tuteur, $photo === '' ? null : $photo, $di, $cid, $remiseMensuelle ?? 0.0, $sexeNew]);
            } catch (\PDOException $e) {
                if (str_contains($e->getMessage(), '1062') || str_contains($e->getMessage(), 'Duplicate entry')) {
                    if (str_contains(strtolower($e->getMessage()), 'cin')) {
...
// Pre-compute the next N° Inscription for the new-stagiaire form preview
$_previewYear = date('Y');
$_previewSt = $pdo->prepare("SELECT COALESCE(MAX(CAST(SUBSTRING_INDEX(num_inscri, '-', -1) AS UNSIGNED)), 0) FROM stagiaires WHERE num_inscri LIKE ? AND num_inscri REGEXP '^INS-[0-9]{4}-[0-9]{5}$'");
$_previewSt->execute(['INS-' . $_previewYear . '-%']);
$nextNumInscri = 'INS-' . $_previewYear . '-' . str_pad((string)((int)$_previewSt->fetchColumn() + 1), 5, '0', STR_PAD_LEFT);
            ...
            <form method="post" class="modal-form" action="stagiaires.php" id="stagiaire-form">
                ...
                <div class="modal-section-grid">
                    <fieldset class="modal-fieldset">
                        ...
                        <label>N° Inscription
                            <input type="text" name="num_inscri" maxlength="32" readonly
                                style="opacity:0.55;cursor:not-allowed;background:rgba(255,255,255,0.03);"
                                value="<?= $edit ? h($edit['num_inscri']) : h($nextNumInscri) ?>">
                            <?php if (!$edit): ?>
                                <span style="font-size:0.72rem;color:#a1a1aa;margin-top:0.2rem;display:block;">Généré automatiquement — non modifiable.</span>
                            <?php endif; ?>
                        </label>
