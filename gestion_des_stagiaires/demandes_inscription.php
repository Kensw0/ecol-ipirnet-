...
/* ──────────────────────────────────────────────────────────────
   POST HANDLERS — Accept / Refuse
────────────────────────────────────────────────────────────── */
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    csrf_verify();

    /* ── EDIT EXISTING PRÉ-INSCRIPTION ── */
    if (!empty($_POST['modifier_id']) && (int)$_POST['modifier_id'] > 0) {
        $mid    = (int)$_POST['modifier_id'];
    ...
    /* ── ADD NEW PRÉ-INSCRIPTION (secretary enters form data) ── */
    if (isset($_POST['nouvelle_preinscription'])) {
        ...
        // id_filiere: visitor expresses interest in a filière only (class assigned on acceptance)
        $filId = (int)($_POST['id_filiere'] ?? 0);
        // Fallback: derive from first checked diplôme checkbox
        if ($filId === 0 && !empty($_POST['diplomes_filieres'])) {
            $filId = (int)(array_values((array)$_POST['diplomes_filieres'])[0] ?? 0);
        }
        ...
        // CIN: required, must match 2 letters + 6 digits
        if ($cin === '') {
            flash_set('Le CIN est obligatoire.');
            redirect('demandes_inscription.php');
        }
        ...
        if ($tel !== '' && !preg_match('/^(\+?212|0)[5-7][0-9]{8}$/', preg_replace('/\s/', '', $tel))) {
        ...
        // ── CIN duplicate check against stagiaires (same logic as validation flow) ──
        if ($cin !== '') {
            $chkCinNew = $pdo->prepare('SELECT id_stagiaire, nom, prenom, num_inscri FROM stagiaires WHERE cin = ?');
            $chkCinNew->execute([$cin]);
            $existStagNew = $chkCinNew->fetch();
            if ($existStagNew) {
                $nomExN = h($existStagNew['prenom'] . ' ' . $existStagNew['nom']);
                $numExN = h($existStagNew['num_inscri']);
                flash_set(
                    "⚠️ Ce CIN est déjà utilisé par le stagiaire {$nomExN} (N° {$numExN}). " .
                    "Veuillez mettre à jour son dossier.",
                    'warning'
                );
                redirect('stagiaires.php?highlight=' . (int)$existStagNew['id_stagiaire']);
            }
        }
    ...
    if (isset($_POST['accepter_id'])) {
        ...
        try {
            $st = $pdo->prepare('SELECT * FROM pre_inscription WHERE id_demande = ? AND statut = ? FOR UPDATE');
            ...
            if ($cin_val !== '') {
                $chkCin = $pdo->prepare('SELECT id_stagiaire, nom, prenom, num_inscri FROM stagiaires WHERE cin = ?');
                $chkCin->execute([$cin_val]);
                $existingStag = $chkCin->fetch();
                if ($existingStag) {
                    // CIN belongs to an existing stagiaire — do NOT convert, leave pre-inscription as en_attente
                    $pdo->rollBack();
                    $nomExist = h($existingStag['prenom'] . ' ' . $existingStag['nom']);
                    $numExist = h($existingStag['num_inscri']);
                    flash_set(
                        "⚠️ Ce CIN appartient déjà au stagiaire {$nomExist} (N° {$numExist}). " .
                        "La pré-inscription reste en attente — veuillez corriger le CIN.",
                        'warning'
                    );
                    redirect('stagiaires.php?highlight=' . (int)$existingStag['id_stagiaire']);
                }
            }
            ...
            $newNumInscri = 'INS-' . $year . '-' . str_pad((string)($maxNum + 1), 5, '0', STR_PAD_LEFT);

            // Class must be explicitly chosen by the secretary — no automatic fallback
            $finalClasseId = isset($_POST['override_id_classe']) && (int)$_POST['override_id_classe'] > 0
            ...
            // Verify the chosen class belongs to the correct filière
            $chkCls = $pdo->prepare('SELECT id_classe FROM classes WHERE id_classe = ? AND id_filiere = ?');
            $chkCls->execute([$finalClasseId, (int)$d['id_filiere']]);
            if (!$chkCls->fetchColumn()) {
                $pdo->rollBack();
            ...
            $ins->execute([
                $newNumInscri,
                $d['cin'] ?: null, $d['nom'], $d['prenom'],
                $d['date_naissance'] ?: null, $d['adresse'] ?: null,
                $d['email'] ?: null, $d['telephone'] ?: null,
                $d['telephone_parent'] ?: null, $d['nom_tuteur'] ?: null,
                $hash, null, $di, $finalClasseId,
            ]);
            $newStagiaireId = (int)$pdo->lastInsertId();
            $pdo->prepare('UPDATE pre_inscription SET statut = ?, date_decision = NOW(), id_stagiaire_cree = ? WHERE id_demande = ?')
                ->execute(['converti', $newStagiaireId, $idDem]);
            $pdo->commit();
            flash_set('Pré-inscription acceptée — stagiaire créé (' . $newNumInscri . ').');
        } catch (Throwable $e) {
