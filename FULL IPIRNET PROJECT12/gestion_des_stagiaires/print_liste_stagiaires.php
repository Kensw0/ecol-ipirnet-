...
$allowedSort = ['nom', 'num_inscri', 'filiere', 'classe'];
if (!in_array($sortKey, $allowedSort, true)) {
    $sortKey = 'nom';
}

$orderBy = match ($sortKey) {
    'num_inscri' => 's.num_inscri, s.nom, s.prenom',
...
$sql = "SELECT s.id_stagiaire, s.num_inscri, s.nom, s.prenom, s.date_inscription,
        ...
        <tbody>
            ...
            <?php else: ?>
                <?php $i = 0; foreach ($rows as $r): $i++; ?>
                    <tr>
                        <td class="code"><?= h((string) ($r['num_inscri'] ?? sprintf('%02d', $i))) ?></td>
