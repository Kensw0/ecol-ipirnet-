...
$niveau     = "1ère Année"; // Placeholder for Niveau as per picture
    ...
    <table class="header-table">
        <tr>
            ...
            <td style="width: 20%; text-align:right;">
                <div align="right">
                    <div style="width: 70px; height: 70px; border: 2px solid #000; border-radius: 50%; display:flex; align-items:center; justify-content:center; font-weight:bold; font-size:11px;">ACCREDITE</div>
                </div>
            </td>
        </tr>
    </table>
    ...
    <table class="grades-table">
        <thead>
            <tr>
                ...
                <th rowspan="2">Moyenne UF</th>
                <th rowspan="2">Observations</th>
            </tr>
            <tr>
                <?php if ($mode === 'examen' || $mode === 'combined'): ?>
                    <th style="width:70px;">Théorique</th>
                    <th style="width:70px;">Pratique</th>
                <?php endif; ?>
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                <td class="module-name"><?= h(gds_module_label((string)$r['nom_module'])) ?></td>
                <td class="coeff"><?= (int)$r['coefficient'] ?></td>
                
                <?php if ($mode === 'controle' || $mode === 'combined'): ?>
                    <td><?= h($fmtNote($r['note_controle'])) ?></td>
                <?php endif; ?>
                
                <?php if ($mode === 'examen' || $mode === 'combined'): ?>
                    <td><?= h($fmtNote($r['note_theorique'])) ?></td>
                    <td><?= h($fmtNote($r['note_pratique'])) ?></td>
                <?php endif; ?>
                
                <td style="font-weight:bold;"><?= h($fmtNote($r['moyenne_module'])) ?></td>
                <td style="font-size:11px;"><?= $getObs($r['moyenne_module']) ?></td>
            </tr>
            <?php endforeach; ?>
            
            <?php if (!$rows): ?>
                <tr>
                    <td colspan="<?= $totalCols + 1 ?>" style="padding:20px; font-style:italic;">Aucune formation enregistrée.</td>
                </tr>
            <?php endif; ?>
        </tbody>
        ...
        <tbody>
            <!-- Mode Averages -->
            <tr>
                <td colspan="2" class="bottom-row" style="text-align:center;">Moyenne des notes</td>
                
                <?php if ($mode === 'controle' || $mode === 'combined'): ?>
                    <td class="bottom-row"><?= h($fmtNote($avgC)) ?></td>
                <?php endif; ?>
                
                <?php if ($mode === 'examen' || $mode === 'combined'): ?>
                    <td class="bottom-row"><?= h($fmtNote($avgT)) ?></td>
                    <td class="bottom-row"><?= h($fmtNote($avgP)) ?></td>
                <?php endif; ?>
                
                <!-- Blank for Moyenne UF and Observations on this row to match picture -->
            ...
            <tr>
                <?php
                    // Adjust colspans to center "Moyenne Générale".
                    // The text label spans over Module+Coef + Controle
                    // The value spans over Theo + Prac
                    
                    $labelCols = 2; // Module + Coeff
                    if ($mode === 'controle' || $mode === 'combined') $labelCols += 1; // Controle
                    
                    $valCols = 1; // Moyenne UF
                    if ($mode === 'examen' || $mode === 'combined') $valCols += 2; // Theo + Pra
                ?>
                <td colspan="<?= $labelCols ?>" class="bottom-row" style="text-align:center;">
                    Moyenne Générale de la <?= h($niveau) ?> :
                </td>
                <td colspan="<?= $valCols ?>" class="bottom-row" style="text-align:center; font-size:14px;">
                    <?= $gm !== null ? h($fmtNote($gm)) : '—' ?>
                </td>
                <td style="background:#e8e8e8; border-top:2px solid #000;"></td>
            </tr>
            
            <!-- Jury Decision -->
            <tr>
                <td colspan="<?= $labelCols ?>" class="bottom-row" style="text-align:center;">
                    Proposition du jury d'examens :
                </td>
                <td colspan="<?= $valCols + 1 ?>" class="bottom-row" style="text-align:center; font-size:14px;">
                    <?= h($decision) ?>
                </td>
            </tr>
        </tbody>
        ...
        <tr>
            <td>
                <div class="signature-box">
                    <div class="title">EMARGEMENT DU PRESIDENT DU JURY</div>
                </div>
            </td>
            <td>
                <div class="signature-box">
                    <div class="title">EMARGEMENT DU DIRECTEUR PEDAGOGIQUE</div>
                </div>
            </td>
        </tr>
    </table>
